...
export class SseManager extends Effect.Service<SseManager>()("SseManager", {
  effect: Effect.gen(function* () {
    ...
    const notifyUser = ({ event, userId }: { userId: UserId; event: Events }) =>
      Effect.gen(function* () {
        ...
        const encodedEvent = yield* Schema.encode(Schema.parseJson(Events))(event).pipe(
          Effect.orDie,
        );
    ...
    // const notifyCurrentUser = (event: Events) =>
    //   Effect.gen(function* () {
    //     const currentUser = yield* CurrentUser;
    //     yield* notifyUser({ event, userId: currentUser.userId });
    //   });

    const notifyAll = ({ event }: { event: Events }) =>
      Effect.gen(function* () {
